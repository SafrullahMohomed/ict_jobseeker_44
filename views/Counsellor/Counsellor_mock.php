<!DOCTYPE html>
<html lang="en">

<head>
    <meta charset="UTF-8">
    <meta http-equiv="X-UA-Compatible" content="IE=edge">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <link rel="stylesheet" href="/ict_jobseeker_44/views/CSS/Counsellor/Counsellor_home_js.css">
    <link rel="stylesheet" href="/ict_jobseeker_44/views/CSS/Normalize/Normalize.css">
    <link rel="stylesheet" href="https://cdnjs.cloudflare.com/ajax/libs/font-awesome/4.7.0/css/font-awesome.min.css">
    <title>Document</title>
</head>

<body>

<div class="header_div">
    <?php include './views/Header.php' ?>
</div>
    <div class="main_container">
        <div class="counsellor_title"> Find&nbsp; Counsellor</div>
        <div class="counsellor_text"> Contact Counsellors and find suitable jobs for your qualificaitons.</div>

        <!-- find mock interviews part -->
        <a href="#">
            <div class="mock_interviews">Find Mock interviews providing counsellors</div>
        </a>

        <!-- search part -->
        <div class="search-counsellor">
            <input type="text" name="search-counsellors" id="search-counsellors" placeholder="Search Counsellors">
            <button>Search</button>
        </div>
        <!-- search part ends here -->

        <!-- counsellor list starts here -->
        <!-- counsellor 1 -->
        <a href="<?php echo URL ?>Counsellor/Counsellor_profile_js">
            <div class="counsellor">
                <div class="counsellor_profile"><img src="/ict_jobseeker_44/views/images/Counsellor/Counselling.png" alt="profile image"></div>
                <div class="counsellor_name">Counsellor1</div>
                <!-- <div class="star">
                <div class="star_icon"><span class="fa fa-star checked" style="font-size: 40px;"></span></div>
                <div class="star_value">4.2</div>
            </div> -->
                <div class="mi_provider">
                    <div class="mk_interview">Mock interviews</div>
                    <div class="tick"><i class='fa fa-check-circle' style="font-size: 40px; color:green;"></i></div>
                </div>

            </div>
        </a>

        <!-- counsellor 1 ends here -->

        <!-- counsellor 2 -->
        <a href="#">
            <div class="counsellor">
                <div class="counsellor_profile"><img src="/ict_jobseeker_44/views/images/Counsellor/Counselling.png" alt="profile image"></div>
                <div class="counsellor_name">Counsellor3</div>
                <!-- <div class="star">
                <div class="star_icon"><span class="fa fa-star checked" style="font-size: 40px;"></span></div>
                <div class="star_value">3.8</div>
            </div> -->
                <div class="mi_provider">
                    <div class="mk_interview">Mock interviews</div>
                    <div class="tick"><i class='fa fa-check-circle' style="font-size: 40px; color:green;"></i></div>
                </div>
            </div>

    </div>
    </a>

    <!-- counsellor 2 ends here -->

    <!-- counsellor 3 -->
    <a href="#">
        <div class="counsellor">
            <div class="counsellor_profile"><img src="/ict_jobseeker_44/views/images/Counsellor/Counselling.png" alt="profile image"></div>
            <div class="counsellor_name">Counsellor4</div>
            <!-- <div class="star">
                <div class="star_icon"><span class="fa fa-star checked" style="font-size: 40px;"></span></div>
                <div class="star_value">4.8</div>
            </div> -->
            <div class="mi_provider">
                <div class="mk_interview">Mock interviews</div>
                <div class="tick"><i class='fa fa-check-circle' style="font-size: 40px; color:green;"></i></div>
            </div>

        </div>
    </a>

    <!-- counsellor 3 ends here -->

    <!-- counsellor 4 -->
    <a href="#">
        <div class="counsellor">
            <div class="counsellor_profile"><img src="/ict_jobseeker_44/views/images/Counsellor/Counselling.png" alt="profile image"></div>
            <div class="counsellor_name">Counsellor7</div>
            <!-- <div class="star">
                <div class="star_icon"><span class="fa fa-star checked" style="font-size: 40px;"></span></div>
                <div class="star_value">2.9</div>
            </div> -->
            <div class="mi_provider">
                <div class="mk_interview">Mock interviews</div>
                <div class="tick"><i class='fa fa-check-circle' style="font-size: 40px; color:green;"></i></div>
            </div>

        </div>
    </a>

    <!-- counsellor 4 ends here -->

    <!-- counsellor 5 -->
    <a href="#">
        <div class="counsellor">
            <div class="counsellor_profile"><img src="/ict_jobseeker_44/views/images/Counsellor/Counselling.png" alt="profile image"></div>
            <div class="counsellor_name">Counsellor10</div>
            <!-- <div class="star">
                <div class="star_icon"><span class="fa fa-star checked" style="font-size: 40px;"></span></div>
                <div class="star_value">3.0</div>
                </div> -->
            <div class="mi_provider">
                <div class="mk_interview">Mock interviews</div>
                <div class="tick"><i class='fa fa-check-circle' style="font-size: 40px; color:green;"></i></div>
            </div>
        </div>
        </div>
    </a>



    <!-- counsellor 5 ends here -->

    <!-- counsellor 6 -->
    <a href="#">
        <div class="counsellor">
            <div class="counsellor_profile"><img src="/ict_jobseeker_44/views/images/Counsellor/Counselling.png" alt="profile image"></div>
            <div class="counsellor_name">Counsellor12</div>
            <!-- <div class="star">
                <div class="star_icon"><span class="fa fa-star checked" style="font-size: 40px;"></span></div>
                <div class="star_value">3.2</div>
            </div> -->
            <div class="mi_provider">
                <div class="mk_interview">Mock interviews</div>
                <div class="tick"><i class='fa fa-check-circle' style="font-size: 40px; color:green;"></i></div>
            </div>
        </div>

        </div>
    </a>

    <!-- counsellor 6 ends here -->

    <!-- counsellor 7 -->
    <a href="#">
        <div class="counsellor">
            <div class="counsellor_profile"><img src="/ict_jobseeker_44/views/images/Counsellor/Counselling.png" alt="profile image"></div>
            <div class="counsellor_name">Counsellor20</div>
            <!-- <div class="star">
                <div class="star_icon"><span class="fa fa-star checked" style="font-size: 40px;"></span></div>
                <div class="star_value">3.6</div>
            </div> -->
            <div class="mi_provider">
                <div class="mk_interview">Mock interviews</div>
                <div class="tick"><i class='fa fa-check-circle' style="font-size: 40px; color:green;"></i></div>
            </div>

        </div>
    </a>

    <!-- counsellor 7 ends here -->

    <!-- counsellor 8 -->
    <a href="#">
        <div class="counsellor">
            <div class="counsellor_profile"><img src="/ict_jobseeker_44/views/images/Counsellor/Counselling.png" alt="profile image"></div>
            <div class="counsellor_name">Counsellor27</div>
            <!-- <div class="star">
                <div class="star_icon"><span class="fa fa-star checked" style="font-size: 40px;"></span></div>
                <div class="star_value">4.9</div>
            </div> -->
            <div class="mi_provider">
                <div class="mk_interview">Mock interviews</div>
                <div class="tick"><i class='fa fa-check-circle' style="font-size: 40px; color:green;"></i></div> </i>
            </div>
        </div>

        </div>
    </a>

    <!-- counsellor 8 ends here -->

    <!-- counsellor 9 -->
    <a href="#">
        <div class="counsellor">
            <div class="counsellor_profile"><img src="/ict_jobseeker_44/views/images/Counsellor/Counselling.png" alt="profile image"></div>
            <div class="counsellor_name">Counsellor28</div>
            <!-- <div class="star">
                <div class="star_icon"><span class="fa fa-star checked" style="font-size: 40px;"></span></div>
                <div class="star_value">2.8</div>
            </div> -->
            <div class="mi_provider">
                <div class="mk_interview">Mock interviews</div>
                <div class="tick"><i class='fa fa-check-circle' style="font-size: 40px; color:green;"></i></div>
            </div>
        </div>

        </div>
    </a>

    <!-- counsellor 9 ends here -->

    <!-- counsellor 10 -->
    <a href="#">
        <div class="counsellor">
            <div class="counsellor_profile"><img src="/ict_jobseeker_44/views/images/Counsellor/Counselling.png" alt="profile image"></div>
            <div class="counsellor_name">Counsellor32</div>
            <!-- <div class="star">
                <div class="star_icon"><span class="fa fa-star checked" style="font-size: 40px;"></span></div>
                <div class="star_value">3.9</div>
            </div> -->
            <div class="mi_provider">
                <div class="mk_interview">Mock interviews</div>
                <div class="tick"><i class='fa fa-check-circle' style="font-size: 40px; color:green;"></i></div>
            </div>

        </div>
    </a>

    <!-- counsellor 10 ends here -->


    <!-- load more button -->
    <div class="load-more">
        <button>Load More</button>
    </div>


    </div>
    </div>

<!-- footer -->
<?php include './views/Footer.php' ?>

</body>

</html>